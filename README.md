...
轻量级 HTTP 客户端，类似 Postman。支持单次请求、CSV 批量测试、cURL 导入导出，可打包为 Windows 桌面应用，双击即用，无需常驻开发服务器。

## 功能

- HTTP 请求：GET / POST / PUT / DELETE / PATCH / HEAD / OPTIONS
- Headers、Body（JSON / raw / x-www-form-urlencoded / form-data）
- 响应展示：状态码、Headers、Body（含二进制 base64）
- 请求历史（localStorage，最多 200 条）
- cURL 导入 / 复制
- CSV Batch：按 CSV 行批量发请求，URL / Body 支持 `{{列名}}` 模板
- 一键清除 Headers 与 Body
- Windows 桌面版（Tauri）：内置 Rust 代理，绕过浏览器 CORS
...
├── src/
│   ├── App.jsx         # 主界面
│   ├── components/     # Headers、Body、Response、CSV Batch 等
│   └── utils/          # 代理封装、cURL 解析、历史记录
...
说明 `/api/proxy` 返回了 HTML 而非 JSON。请使用 `pnpm dev` 启动完整开发环境，或使用桌面版 `app.exe`。

**`pnpm tauri:build` 报 `link.exe not found`**

未安装或未配置 MSVC。安装 Visual Studio Build Tools 后，使用 `pnpm tauri:build:win` 打包。
